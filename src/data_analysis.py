import pandas as pd
import numpy as np
from pre_processing import df_transform_experimental

def calculate_adversarial(df_train, df_test):
    params = {
        'size_encoding': 'one_hot'
    }
    df_train = df_transform_experimental(df_train, params)
    df_test = df_transform_experimental(df_test, params)

    X_adv = pd.concat([df_train.drop(columns=['track_id'], errors='ignore'),
